# -*- coding: utf-8 -*-
# -----------------------------------------------------------------------------
# E-commerce Data Analyst App - V10.0 (Comprehensive Charts)
#
# This version significantly expands the number of insightful charts on all
# three tabs to provide deeper, more actionable business intelligence.
# -----------------------------------------------------------------------------
...
def create_sales_layout():
    return dbc.Container([
        ...
        dbc.Row([
            dbc.Col(dcc.Graph(id='sales-by-weekday-chart'), width=12),
        ])
    ], fluid=True)
...
def create_delivery_layout():
    return dbc.Container([
        ...
        dbc.Row([
            dbc.Col(dcc.Graph(id='delivery-time-dist-chart'), width=6),
            dbc.Col(dcc.Graph(id='on-time-by-city-chart'), width=6),
        ]),
    ], fluid=True)
...
app.layout = dbc.Container([
    ...
    html.H1("Pharma Analytics Hub - v10.0 (Comprehensive)", className='text-center text-primary mb-4'),
...
# Callback to render the correct tab content by calling the layout functions
...
@app.callback(
    [Output('kpi-net-sales', 'children'), Output('kpi-total-discount', 'children'),
     ...
     Output('sales-by-weekday-chart', 'figure')],
...
def update_sales_dashboard(selected_location, start_date, end_date):
    ...
    weekday_sales = filtered_df.groupby('weekday')['netsale'].sum().reset_index()
    weekday_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    weekday_sales['weekday'] = pandas.Categorical(weekday_sales['weekday'], categories=weekday_order, ordered=True)
    weekday_sales = weekday_sales.sort_values('weekday')
    sales_by_weekday_fig = px.bar(weekday_sales, x='weekday', y='netsale', title='Sales by Day of the Week')

    return kpi_netsales_card, kpi_discount_card, kpi_orders_card, sales_over_time_fig, sales_by_cat_fig, top_prod_fig, sales_by_weekday_fig

# Callback for Delivery Dashboard
@app.callback(
    [Output('kpi-on-time-delivery', 'children'), Output('kpi-avg-delivery-time', 'children'),
     ...
     Output('partner-performance-chart', 'figure'), Output('delivery-time-dist-chart', 'figure'),
     Output('on-time-by-city-chart', 'figure')],
...
def update_delivery_dashboard(start_date, end_date):
    ...
    delivery_time_dist_fig = px.histogram(filtered_df, x='delivery_time_days', nbins=15, title='Delivery Time Distribution', labels={'delivery_time_days': 'Days to Deliver'})
    
    on_time_by_city = filtered_df.groupby('city')['on_time'].value_counts(normalize=True).unstack().fillna(0) * 100
    on_time_by_city.rename(columns={True:'On-Time', False:'Delayed'}, inplace=True)
    on_time_by_city_fig = px.bar(on_time_by_city, barmode='group', title='On-Time vs. Delayed by City', labels={'value': 'Percentage', 'city': 'City'})

    return kpi_on_time_card, kpi_avg_time_card, kpi_total_del_card, status_chart_fig, partner_perf_chart_fig, delivery_time_dist_fig, on_time_by_city_fig

# Callback for Customer Dashboard
@app.callback(
