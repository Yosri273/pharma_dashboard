# -*- coding: utf-8 -*-
# -----------------------------------------------------------------------------
# UI Layouts Module - V23.0 (Advanced Chart Integration)
#
# ADDED: New graph card placeholders to each dashboard tab for the new
#        strategic charts suggested in the enhancement plan.
# -----------------------------------------------------------------------------
...
# ... (All helper functions like create_filter_options and create_kpi_card remain unchanged) ...
...
def create_datatable_card(table_id: str, title: str, width: int = 6, lg_width: int = None) -> dbc.Col:
    """Creates a Bootstrap Column containing a DataTable in a Card."""
    lg_col_width = lg_width if lg_width is not None else width
...
def create_sales_layout() -> dbc.Container:
    # ... (Filter and KPI rows are unchanged) ...
    ...
    return dbc.Container([
        ...
        dbc.Row([
            create_kpi_card("Total Revenue", "kpi-total-revenue", "primary"), create_kpi_card("Gross Margin", "kpi-gross-margin", "success"), create_kpi_card("Net Profit", "kpi-net-profit", "dark"),
        ]),
        dbc.Row([
            create_kpi_card("Total Orders", "kpi-total-orders", "info"), create_kpi_card("Avg Order Value", "kpi-aov", "secondary"), create_kpi_card("Return Rate", "kpi-return-rate", "danger"),
        ]),
        dbc.Row([create_graph_card('sales-funnel-chart', width=12)]),
        dbc.Row([create_graph_card('sales-over-time-chart', width=12)]),
        # --- NEW: Row for Advanced Sales Charts ---
        html.Hr(className="my-4"),
        dbc.Row([html.H4("Advanced Sales Analytics", className="mb-3 text-center")]),
        dbc.Row([
            create_graph_card('period-growth-chart', title="Period-over-Period Growth"),
            create_graph_card('price-volume-chart', title="Price vs. Volume Analysis")
        ]),
        html.Hr(className="my-4"),
        # --- END NEW ROW ---
...
def create_delivery_layout() -> dbc.Container:
    ...
    partner_opts, region_opts = delivery_df['deliverypartner'].unique(), delivery_df['city'].unique()
    return dbc.Container([
        dbc.Card(dbc.CardBody([
             dbc.Row([
                 dbc.Col([html.Label("Date Range:"), dcc.DatePickerRange(id='delivery-date-picker', min_date_allowed=delivery_df['date'].min(), max_date_allowed=delivery_df['date'].max(), start_date=delivery_df['date'].min(), end_date=delivery_df['date'].max(), className="d-block")], width=12, lg=4, className="mb-2"),
                 dbc.Col([html.Label("Region (City):"), dcc.Dropdown(id='delivery-region-filter', options=create_multi_filter_options(region_opts), value=['All'], multi=True, clearable=False)], width=12, lg=3, className="mb-2"),
                 dbc.Col([html.Label("Delivery Partner:"), dcc.Dropdown(id='delivery-partner-filter', options=create_filter_options(partner_opts), value='All', clearable=False)], width=12, lg=3, className="mb-2"),
                 dbc.Col([html.Label("Actions", style={'visibility': 'hidden'}), dbc.ButtonGroup([dbc.Button("Apply", id="delivery-apply-btn", color="primary"), dbc.Button("Export PDF", id="delivery-export-btn", color="secondary")], className="w-100")], width=12, lg=2, className="mb-2 align-self-end"),
             ], align="bottom"),
        ]), className="mb-4"),
        dbc.Row([
            create_kpi_card("On-Time Rate", "kpi-on-time-delivery", "success", width=3, md_width=6), create_kpi_card("Failed Delivery Rate", "kpi-failed-delivery", "danger", width=3, md_width=6),
            create_kpi_card("Avg. Delivery Time", "kpi-avg-delivery-time", "warning", width=3, md_width=6), create_kpi_card("Avg. Cost per Delivery", "kpi-avg-delivery-cost", "info", width=3, md_width=6),
        ]),
        dbc.Row([create_graph_card('delivery-pipeline-chart', width=12)]),
        # --- NEW: Row for Advanced Logistics Chart ---
        html.Hr(className="my-4"),
        dbc.Row([
            create_graph_card('partner-matrix-chart', title="Partner Cost vs. Performance Matrix", width=12)
        ]),
        html.Hr(className="my-4"),
        # --- END NEW ROW ---
...
def create_customer_layout() -> dbc.Container:
    # ... (Filter and KPI rows are unchanged from the last step) ...
    ...
    return dbc.Container([
        ...
        dbc.Row([
            create_kpi_card("Total Customers", "kpi-total-customers", "primary", width=3, md_width=6), create_kpi_card("Active Customers", "kpi-active-customers", "info", width=3, md_width=6),
            create_kpi_card("Retention Rate", "kpi-retention-rate", "success", width=3, md_width=6), create_kpi_card("Repeat Purchase Rate", "kpi-repeat-purchase-rate", "success", width=3, md_width=6),
        ]),
        ...
        # --- NEW: Row for Advanced Customer Chart ---
        html.Hr(className="my-4"),
        dbc.Row([
            create_graph_card('rfm-bubble-chart', title="RFM Segment Analysis", width=12)
        ]),
        html.Hr(className="my-4"),
        # --- END NEW ROW ---
        dbc.Card(dbc.CardBody([
...
def create_marketing_layout() -> dbc.Container:
    ...
    return dbc.Container([
        ...
        dbc.Row([
            create_kpi_card("Total Ad Spend", "kpi-total-ad-spend", "info", width=3, md_width=6), create_kpi_card("Overall ROAS", "kpi-avg-roas", "primary", width=3, md_width=6),
            create_kpi_card("Average CPA (CAC)", "kpi-avg-cpa", "warning", width=3, md_width=6), create_kpi_card("CLV to CAC Ratio", "kpi-clv-cac-ratio", "success", width=3, md_width=6),
        ]),
        dbc.Row([create_kpi_card("Attributed Conversions", "kpi-total-conversions", "dark", width=12)]),
        # --- NEW: Row for Advanced Marketing Chart ---
        html.Hr(className="my-4"),
        dbc.Row([
            create_graph_card('clv-by-channel-chart', title="Average Customer Lifetime Value by Acquisition Channel", width=12)
        ]),
        html.Hr(className="my-4"),
        # --- END NEW ROW ---
...
def create_profit_layout() -> dbc.Container:
    ...
    return dbc.Container([
        ...
        dbc.Row([
            create_kpi_card("Total Net Profit", "kpi-total-net-profit", "success"), create_kpi_card("Average Profit Margin", "kpi-avg-profit-margin", "primary"),
            create_kpi_card("Profit Lost to Returns", "kpi-profit-lost-returns", "danger"),
        ]),
        # --- NEW: Row for Advanced Profit Chart ---
        html.Hr(className="my-4"),
        dbc.Row([
            create_graph_card('profit-waterfall-chart', title="Profitability Waterfall Analysis", width=12)
        ]),
        html.Hr(className="my-4"),
        # --- END NEW ROW ---
        dbc.Row([
