# -*- coding: utf-8 -*-
# -----------------------------------------------------------------------------
# Interactive Callbacks Module - V23.2 (Export Button Bug Fix)
#
# BUG FIX: Uncommented "if n_clicks is None: raise PreventUpdate" in all five
# PDF export callbacks to prevent them from firing on tab-switch/render.
# -----------------------------------------------------------------------------
...
# Path to the state file generated by the alert monitor
...
def register_callbacks(app):
    ...
    # --- NEW CALLBACK FOR DISPLAYING ACTIVE ALERTS ---
    @app.callback(
    ...
    def update_active_alert_display(n):
        """
        Polls the alert state file and displays a banner for any alert
        that is currently in an active (triggered) state.
        """
        if not os.path.exists(ALERT_STATE_FILE):
            return []  # No file, no alerts
        ...
        alert_banners = []
        for alert_name, state_data in alert_state.items():
            if state_data.get("is_active", False):
                message = state_data.get("message", "No details provided.")
                timestamp = state_data.get("last_triggered", "")
                
                if timestamp:
                    try:
                        ts_pretty = datetime.fromisoformat(timestamp).strftime('%Y-%m-%d %I:%M %p')
                        header = f"{alert_name} (Since: {ts_pretty})"
                    except ValueError:
                        header = f"{alert_name} (At: {timestamp})"
                else:
                    header = alert_name
                ...
                alert_banners.append(
                    dbc.Alert(
                        [
                            html.H5(header, className="alert-heading"),
                            html.P(message)
                        ],
                        color="danger",
                        dismissable=True,
                        duration=45000, # Auto-dismiss after 45 seconds
                        className="mb-3"
                    )
                )
    ...
    # --- Sales Dashboard Callback (Original logic, updated to trigger from 'Apply' button) ---
    @app.callback(
        ...
        [Input('sales-apply-btn', 'n_clicks')], # UPDATED trigger
        [State('channel-filter-dropdown', 'value'),
         ...
         State('sales-region-filter', 'value'), # NEW filter state
         State('sales-category-filter', 'value')] # NEW filter state
    )
    ...
    def export_sales_pdf(n_clicks, selected_channel, start_date, end_date, time_agg, selected_regions, selected_categories):
        # --- BUG FIX ---
        # This check prevents the callback from firing when the layout is drawn
        if n_clicks is None:
            raise PreventUpdate
        # --- END FIX ---
    ...
    # --- Delivery Dashboard Callback (Original logic, updated to trigger from 'Apply' button) ---
    @app.callback(
    ...
    # --- DELIVERY EXPORT CALLBACK (FIXED) ---
    @app.callback(
        Output('download-dashboard-pdf', 'data', allow_duplicate=True),
    ...
    # --- Customer Dashboard Callback (Original logic, updated to trigger from 'Apply' button) ---
    @app.callback(
        ...
        [Input('customer-apply-btn', 'n_clicks')], # UPDATED trigger
        [State('customer-list-selector', 'value'),
         State('customer-date-picker', 'start_date'), # NEW filter state
         State('customer-date-picker', 'end_date'),   # NEW filter state
         State('customer-region-filter', 'value'),  # NEW filter state
         State('customer-segment-filter', 'value')] # NEW filter state
    )
    def update_customer_dashboard(n_clicks, selected_list, start_date, end_date, selected_regions, selected_segments):
        ...
        # --- KPI Calculation (Original, but on filtered data 'dff') ---
        ...
        # --- Table Logic (Original, but on filtered data 'dff') ---
    ...
    # --- CUSTOMER EXPORT CALLBACK (FIXED) ---
    @app.callback(
        Output('download-dashboard-pdf', 'data', allow_duplicate=True),
    ...
    # --- Marketing Dashboard Callback (Original logic, updated to trigger from 'Apply' button) ---
    @app.callback(
        ...
        [Input('marketing-apply-btn', 'n_clicks')], # UPDATED trigger
        [State('marketing-date-picker', 'start_date'), # NEW filter state
         State('marketing-date-picker', 'end_date'),   # NEW filter state
         State('marketing-channel-filter', 'value')] # NEW filter state
    )
    def update_marketing_dashboard(n_clicks, start_date, end_date, selected_channel):
        ...
        # --- Figure Generation (Original, but on filtered data 'dff') ---
    ...
    # --- MARKETING EXPORT CALLBACK (FIXED) ---
    @app.callback(
        Output('download-dashboard-pdf', 'data', allow_duplicate=True),
    ...
    # --- Profit Dashboard Callback (Original logic, updated to trigger from 'Apply' button) ---
    @app.callback(
        ...
        [Input('profit-apply-btn', 'n_clicks')], # UPDATED trigger
        [State('profit-date-picker', 'start_date'),  # NEW filter state
         State('profit-date-picker', 'end_date'),    # NEW filter state
         State('profit-region-filter', 'value'),    # NEW filter state
         State('profit-category-filter', 'value')] # NEW filter state
    )
    def update_profit_dashboard(n_clicks, start_date, end_date, selected_regions, selected_categories):
        ...
        # --- Recommendations Logic (Original, but on filtered data 'dff') ---
        recommendations = []
    ...
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    # --- PREDICTIVE ANALYTICS CALLBACKS (MERGED) ---
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

    # --- Forecast Simulation Callback (Original) ---
    @app.callback(
    ...
    def update_forecast_simulation(n_clicks, forecast_days, promo_pct):
        ...
        # Check against the correct model path
    ...
    # --- NEW: CHURN TAB FALLBACKS AND TRAINING TRIGGERS ---
    
    def _generate_synthetic_data():
        """(Helper function to generate fallback data if DB is empty)"""
    ...
    def _run_churn_training_job():
        """ (Helper function to run the churn training pipeline from within the app) """
        try:
    ...
    # --- NEW: MASTER CALLBACK 1: Renders Churn Tab Content (Replaces original churn callback) ---
    @app.callback(
    ...
    def render_churn_tab_content(active_tab, training_signal):
        """
        This callback dynamically renders the *entire* content of the churn tab.
        1. Checks if model files exist.
        2. If NO, it returns a layout with a "Run Training" button.
        3. If YES, it loads the models, runs the logic (same as original callback),
           and returns the full layout of KPIs, charts, and tables.
        """
        ...
        # --- FALLBACK CASE: Models do not exist ---
        if not models_exist:
        ...
        # --- SUCCESS CASE: Models exist, run original logic ---
        ...
        try:
            ...
            # --- Start Original KPI/Chart Logic ---
            ...
            # --- End Original KPI/Chart Logic ---

            # Dynamically build the full layout and return it
            full_layout = [
                dbc.Row([
    ...
    # --- NEW: MASTER CALLBACK 2: Triggers Training Job ---
    @app.callback(
    ...
    def trigger_churn_model_training(n_clicks, current_signal):
        """Triggers the internal training job when the fallback button is clicked."""
        if n_clicks == 0 or n_clicks is None:
            raise PreventUpdate
        ...
        if not success:
            ...
            # Optionally: return an alert to the user here
        
        # Increment the signal store to trigger the render_churn_tab_content callback
